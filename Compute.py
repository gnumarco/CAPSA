...
def get_distinct_elements(cursor):
    if user == "D":
        conv_file = "C:\\Users\\tr5568\\Desktop\\DAYANA\\CAPSA\\Softwares\\CONVERSION MATERIAL -SFAPO CAPSA.xlsx"
    elif user == "M":
        conv_file = "C:\\Users\\gnuma\\Google Drive\\CAPSA\\Softwares\\CONVERSION MATERIAL -SFAPO CAPSA.xlsx"
    elif user == "S":
        conv_file = "C:\\Datos analisis\\CONVERSION MATERIAL -SFAPO CAPSA.xlsx"
    ...
    if mode == 1:
        cursor.execute(
            'SELECT DISTINCT "_BIC_ZMATERIAL","_BIC_ZENSENA2","_BIC_ZCDATA","_BIC_ZFAMAPO"  FROM "_SYS_BIC"."CAPSA_BW_01.ZEP1/ZSLSRPTF1" WHERE "_BIC_ZCDATA" NOT IN (\'0000121702\', \'0000121602\') AND "_BIC_ZENSENA2" NOT IN (\'Z5E005\',\'Z5E008\',\'Z5E013\',\'Z5E018\')')
    ...
    # Mode 3 CRF
        ...
        cursor.execute(
            'SELECT DISTINCT "_BIC_ZMATERIA2","_BIC_ZENSENA","_BIC_ZCDATA"  FROM "_SYS_BIC"."CAPSA_BW_01.ZEP1/ZSD_DE03"')

    # Mode 4 MAS Y MAS
    elif mode == 4:
        cursor.execute(
            'SELECT DISTINCT "_BIC_ZMATERIAL","_BIC_ZENSENA","_BIC_ZCDATA","_BIC_ZFAMAPO"  FROM "_SYS_BIC"."CAPSA_BW_01.ZEP1/ZSLSRPTF1" WHERE "_BIC_ZENSENA" = \'Z5E013\' AND "_BIC_ZCDATA" = \'0000121602\'')
...
def read_station():
    ...
    if user == "D":
        station_file = "C:\\Users\\tr5568\\Desktop\\DAYANA\\CAPSA\\Softwares\\Tabla_estacionalidad fichero carga.xlsx"
    elif user == "M":
...
def createDict_MatCRFtoMatCapsa():
    if user == "D":
        file = "C:\\Users\\tr5568\\Desktop\\DAYANA\\CAPSA\\Softwares\\MAESTRO MATERIALES CRF vs CAPSA.xlsx"
    elif user == "M":
        file = "C:\\Users\\gnuma\\Google Drive\\CAPSA\\Softwares\\MAESTRO MATERIALES CRF vs CAPSA.xlsx"
    elif user == "S":
        file = "C:\\Datos analisis\\MAESTRO MATERIALES CRF vs CAPSA.xlsx"
...
def createDict_MatCapsatoFAMAPO():
    if user == "D":
        file = "C:\\Users\\tr5568\\Desktop\\DAYANA\\CAPSA\\Softwares\\CONVERSION MATERIAL -SFAPO CAPSA.xlsx"
    elif user == "M":
        file = "C:\\Users\\gnuma\\Google Drive\\CAPSA\\Softwares\\CONVERSION MATERIAL -SFAPO CAPSA.xlsx"
    elif user == "S":
        file = "C:\\Datos analisis\\CONVERSION MATERIAL -SFAPO CAPSA.xlsx"

    FAMAPO = pd.read_excel(file)
    dict = {}
    for row in FAMAPO.values:
        dict[str(row[0])] = str(row[1])

    return(dict)


def compute_df_total(station, cursor, entries, df_promo, bar):
    dict_CRFtoMatCapsa = createDict_MatCRFtoMatCapsa()
    print(dict_CRFtoMatCapsa)
    dict_MatCapsatoFAMAPO = createDict_MatCapsatoFAMAPO()
    print(dict_MatCapsatoFAMAPO)
    cpt = 0
    df_total = None
    for ent in entries:
...
def main():
    ...
    cnxn = pyodbc.connect('Driver=HDBODBC;SERVERNODE=172.31.100.155:30041;UID=SAPEP01;PWD=EfiProm2018')
    print("Connection established")

    # Gets a cursor on the server to perform query
    cursor = cnxn.cursor()
    entries = get_distinct_elements(cursor)
    print(entries)
    station = read_station()
    promo = read_promo_file()
    print(promo)
    bar = progressbar.ProgressBar(redirect_stdout=True, max_value=len(entries))
    total_df = compute_df_total(station, cursor, entries, promo, bar)
    total_df = add_canib_data(total_df)
    write_to_file(total_df)
    cnxn.close()
